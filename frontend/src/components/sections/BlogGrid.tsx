    ...
    <section className="py-20 md:py-24">
      <div className="container px-6 md:px-8">
        <div className="text-center max-w-xl mx-auto mb-12 md:mb-14">
          ...
          <p className="mt-3 text-[0.9375rem] md:text-base text-muted">Thoughts on development, design, and digital strategy.</p>
        </div>
          ...
          {posts.map((post, i) => (
            <motion.article key={post.slug} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-30px' }}
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs text-muted flex items-center gap-1"><Calendar className="w-3 h-3" />{post.date}</span>
              </div>
              <h3 className="text-sm md:text-base font-semibold mb-2 group-hover:text-accent transition-colors">{post.title}</h3>
              <p className="text-xs md:text-sm text-muted leading-relaxed">{post.excerpt}</p>
            </motion.article>
          ))}
        </div>
      </div>
